...
@st.cache_resource
def load_models():
    # Tải model Pytorch từ Huggingface (mặc định lưu vào cache ẩn dạng read-only)
    hf_pt_path = hf_hub_download(repo_id="AdamCodd/YOLOv11n-face-detection", filename="model.pt")
    
    # Định nghĩa đường dẫn local để lưu và convert sang ONNX
    local_pt_path = str(ROOT_PATH / "experiment" / "yolo_face.pt")
    local_onnx_path = str(ROOT_PATH / "experiment" / "yolo_face.onnx")
    
    # Thực hiện convert sang ONNX nếu file onnx chưa tồn tại
    if not os.path.exists(local_onnx_path):
        os.makedirs(str(ROOT_PATH / "experiment"), exist_ok=True)
        shutil.copy(hf_pt_path, local_pt_path)
        
        # Load model Pytorch tạm thời để chạy lệnh export
        temp_yolo = YOLO(local_pt_path)
        temp_yolo.export(format="onnx") # Hành động này sẽ tạo ra file yolo_face.onnx
        
        # Dọn dẹp file pt sau khi đã có onnx
        if os.path.exists(local_pt_path):
            os.remove(local_pt_path)

    # Khởi tạo lại model YOLO nhưng sử dụng thẳng file weights .onnx (chạy bằng onnxruntime backend)
    yolo_model = YOLO(local_onnx_path, task='detect')
...
if run_camera:
    ...
    while cap.isOpened() and run_camera:
        ...
        # --- 1. YOLO DETECT MỖI FRAME (Đã chạy bằng backend ONNX) ---
