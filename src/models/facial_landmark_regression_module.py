...
class FacialLandmarkRegressionModule(LightningModule):
    ...
    def _compute_nme(self, pred_keypoints: torch.Tensor, target_landmark: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        pred = pred_keypoints
    ...
    def model_step(self, batch: Any) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        images, landmarks, mask = batch['image'], batch['keypoint_rel'], batch['mask']
